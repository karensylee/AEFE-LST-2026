...
def plot_layered_gantt(avail_df, year=2024, output_path=None):
    ...
    # Build the display labels with combined format
    unique_stations_df = avail_df.select(['display_name', 'SITE_ID']).unique()
    # Sort by SITE_ID numerically
    unique_stations_df = unique_stations_df.with_columns(
        pl.col('SITE_ID').cast(pl.Int32).alias('sort_key')
    ).sort('sort_key')

    # Create label list for y-axis (what gets displayed)
    stations_display = [
        f"{row['display_name']} {row['SITE_ID']}" 
        for row in unique_stations_df.iter_rows(named=True)
    ]

    # Keep y_map keyed by display_name (for matching during plotting)
    display_names = unique_stations_df['display_name'].to_list()
    y_map = {name: i for i, name in enumerate(display_names)}    
